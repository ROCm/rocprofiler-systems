...
build-release()
{
    ...
    verbose-run docker run ${DOCKER_ARGS} --rm -v ${PWD}:/home/rocprof-sys --stop-signal "SIGINT" --env DISTRO=${OS} --env ROCM_VERSION=${ROCM_VERSION} --env VERSION=${CODE_VERSION} --env PYTHON_VERSIONS=\"${PYTHON_VERSIONS}\" --env IS_DOCKER=1 ${CONTAINER} /home/rocprof-sys/scripts/build-release.sh ${@}
}
...
do
    ...
    do
        build-release ${USER}/rocprof-sys:release-base-${TAG}-rocm-${ROCM_VERSION} ${DISTRO}-${VERSION} ${ROCM_VERSION} ${CODE_VERSION} ${SCRIPT_ARGS}
    done
done
