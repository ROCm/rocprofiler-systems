...
__host__ __device__ uint64_t
hashOf(uint64_t a, uint64_t b = 0)
{
    a += uint64_t(1) << 32;
    a += b;
    a ^= a >> 32;
...
castTo<__half>(float x)
{
    return __float2half(x);
}
#if RCCL_BFLOAT16 == 1
template <>
__host__ __device__ hip_bfloat16
castTo<hip_bfloat16>(float x)
{
...
#if RCCL_FLOAT8 == 1
template <>
__host__ __device__ rccl_float8
castTo<rccl_float8>(float x)
{
    return static_cast<rccl_float8>(x);
}
template <>
__host__ __device__ rccl_bfloat8
castTo<rccl_bfloat8>(float x)
{
...
struct ReduceAvg_Base<T, /*integral=*/false>
{
    ...
    {
        using T1 =
            typename std::conditional<(sizeof(T) < sizeof(double)), float, double>::type;
...
template <typename T>
__host__ __device__ T
makeFloat(int sign, int exp, uint64_t mant)
{
    union
    {
        T        ans;
...
__host__ __device__ uint64_t
umul32hi(uint32_t a, uint32_t b)
{
...
__host__ __device__ uint64_t
umul64hi(uint64_t a, uint64_t b)
{
#if HIP_VERSION > 50200000
...
__host__ __device__ int
shuffleRank(int rank_n, int rank_me, uint64_t& rng)
{
...
genSumXY(int rank_n, int rank_me, uint64_t& rng, Uint y_max, Uint& x, Uint& y,
...
)
{
    ...
    x          = (s != 0 && r / s < pn) ? 1 + r / s
                                        : 0;  //  First s*pn ranks contribute partition index +1.
...
template <typename T>
__host__ __device__ T
genInOutFloatSum(bool input_not_output, int rank_n, int rank_me, uint64_t seed,
                 intptr_t index, bool same_sign)
{
    constexpr int exp_lo = 1 + FloatLayout<T>::mantissa_bits;
    constexpr int exp_hi = (1 << FloatLayout<T>::exponent_bits) - 1;
    using uintmant_t =
        typename std::conditional<(8 * sizeof(T) > 32), uint64_t, uint32_t>::type;
    ...
    if(!same_sign && (rank_n + 0) / 2 != 0)
    {
        ...
        genSumXY((rank_n + 0) / 2, rank_me / 2, rng, max_mant, x1_mant, y1_mant,
                 /*avoid_y=*/true);
...
template <typename T>
__host__ __device__ T
genInOutFloatPreMulSum(bool input_not_output, int rank_n, int rank_me, uint64_t seed,
                       intptr_t index)
{
    constexpr int exp_lo = 1 + FloatLayout<T>::mantissa_bits;
    constexpr int exp_hi = (1 << FloatLayout<T>::exponent_bits) - 1;
...
template <typename T>
__host__ __device__ T
genInOutFloatProd(bool input_not_output, int rank_n, int rank_me, uint64_t seed,
                  intptr_t index)
{
    ...
    return makeFloat<T>(input_not_output ? x_sign : y_sign,
...
template <
    typename T, typename ReduceFn,
    typename = typename std::enable_if<!std::is_same<ReduceFn, ReduceNil>::value>::type>
__host__ __device__ void
genInput(T& ans, ReduceFn, int rank_n, int rank_me, uint64_t seed, intptr_t index,
...
__host__ __device__ void
genOutput(T& ans, ReduceFn op, int rank_n, uint64_t seed, intptr_t index,
...
__host__ __device__ void
genInput(T& ans, ReduceNil, int rank_n, int rank_me, uint64_t seed, intptr_t index,
         std::integral_constant<bool, IsIntegral>)
{
...
template <typename T, typename ReduceFn, bool IsIntegral>
__host__ __device__ void
genOutput(T& ans, ReduceNil op, int rank_n, uint64_t seed, intptr_t index,
          std::integral_constant<bool, IsIntegral>)
{
...
template <typename T>
__host__ __device__ void
genInput(T& ans, ReduceSum, int rank_n, int rank_me, uint64_t seed, intptr_t index,
...
template <typename T>
__host__ __device__ void
genOutput(T& ans, ReduceSum, int rank_n, uint64_t seed, intptr_t index,
          std::false_type /*integral*/
)
{
...
template <typename T>
__host__ __device__ void
genInput(T& ans, ReduceProd, int rank_n, int rank_me, uint64_t seed, intptr_t index,
         std::false_type /*integral*/
)
{
...
template <typename T>
__host__ __device__ void
genOutput(T& ans, ReduceProd, int rank_n, uint64_t seed, intptr_t index,
          std::false_type /*integral*/
)
{
...
template <typename T>
__host__ __device__ void
genInput(T& ans, ReducePreMulSum, int rank_n, int rank_me, uint64_t seed, intptr_t index,
         std::true_type integral)
{
...
{
    ans = genInOutFloatPreMulSum<T>(/*input_not_output=*/true, rank_n, rank_me, seed,
                                    index);
}

template <typename T>
__host__ __device__ void
genOutput(T& ans, ReducePreMulSum, int rank_n, uint64_t seed, intptr_t index,
          std::false_type /*integral*/
)
{
...
template <typename T>
__host__ __device__ void
genInput(T& ans, ReduceAvg, int rank_n, int rank_me, uint64_t seed, intptr_t index,
         std::false_type /*integral*/
)
{
    ans = genInOutFloatSum<T>(/*input_not_output=*/true, rank_n, rank_me, seed, index,
...
template <typename T>
__host__ __device__ void
genOutput(T& ans, ReduceAvg, int rank_n, uint64_t seed, intptr_t index,
          std::false_type /*integral*/
)
{
    ans = genInOutFloatSum<T>(/*input_not_output=*/false, rank_n, 0, seed, index,
...
template <typename T>
__host__ __device__ void
genInput(T& ans, ReduceMin, int rank_n, int rank_me, uint64_t seed, intptr_t index,
         std::false_type integral)
{
...
template <typename T>
__host__ __device__ void
genInput(T& ans, ReduceMax, int rank_n, int rank_me, uint64_t seed, intptr_t index,
         std::false_type /*integral*/
)
{
    (void) rank_n;  // silence unused warnings
...
__host__ __device__ T
genInput(ReduceFn op, int rank_n, int rank_me, uint64_t seed, intptr_t index)
{
    T ans;
...
template <typename T, typename ReduceFn>
__host__ __device__ T
genOutput(ReduceFn op, int rank_n, uint64_t seed, intptr_t index)
{
    T ans;
...
template <typename T, typename ReduceFn>
__global__ void
prepareInput2(T* elts, intptr_t elt_n, ReduceFn op, int rank_n, int rank_me,
...
void
prepareInput1(void* elts, intptr_t elt_n, int elt_ty, ReduceOp op, int rank_n,
              int rank_me, uint64_t seed, intptr_t elt_ix0, cudaStream_t stream)
{
    int block_n = std::min<intptr_t>(32, (elt_n + 4 * 512 - 1) / (4 * 512));
#    define CASE_TY(T)                                                                   \
        prepareInput2<<<block_n, 512, 0, stream>>>((T*) elts, elt_n, op, rank_n,         \
                                                   rank_me, seed, elt_ix0);              \
        break;
    switch(elt_ty)
    {
...
#    define CASE_OP(op)                                                                  \
        if(rank_n == 1)                                                                  \
            prepareInput1(elts, elt_n, elt_ty, ReduceNil(), rank_n, rank_me, seed,       \
                          elt_ix0, stream);                                              \
        else                                                                             \
            prepareInput1(elts, elt_n, elt_ty, op, rank_n, rank_me, seed, elt_ix0,       \
                          stream);                                                       \
        break;
    switch(red_op)
    {
...
template <typename T, typename ReduceFn>
__global__ void
prepareExpected2(T* elts, intptr_t elt_n, ReduceFn op, int rank_n, uint64_t seed,
                 intptr_t elt_ix0)
{
    intptr_t i0 = blockIdx.x * (elt_n / gridDim.x);
    i0 += blockIdx.x < elt_n % gridDim.x ? blockIdx.x : elt_n % gridDim.x;
...
template <typename ReduceOp>
void
prepareExpected1(void* elts, intptr_t elt_n, int elt_ty, ReduceOp op, int rank_n,
                 uint64_t seed, intptr_t elt_ix0, cudaStream_t stream)
{
    int block_n = std::min<intptr_t>(32, (elt_n + 4 * 512 - 1) / (4 * 512));
#    define CASE_TY(T)                                                                   \
        prepareExpected2<<<block_n, 512, 0, stream>>>((T*) elts, elt_n, op, rank_n,      \
                                                      seed, elt_ix0);                    \
        break;
    switch(elt_ty)
    {
...
#    define CASE_OP(op)                                                                  \
        if(rank_n == 1)                                                                  \
            prepareExpected1(elts, elt_n, elt_ty, ReduceNil(), rank_n, seed, elt_ix0,    \
                             stream);                                                    \
        else                                                                             \
...
calcSumFloatTolerance(int rank_n, int elt_ty)
{
    float power, coef;
    switch(elt_ty)
    {
...
template <typename T>
__global__ void
verifyPrepared(T const* results, T const* expected, intptr_t elt_n, unsigned tolerance,
               int64_t* bad_elt_n)
{
    intptr_t i0 = blockIdx.x * (elt_n / gridDim.x);
    i0 += blockIdx.x < elt_n % gridDim.x ? blockIdx.x : elt_n % gridDim.x;
...
__global__ void
verifyInline2(T const* results, intptr_t elt_n, ReduceFn op, int rank_n, uint64_t seed,
...
void
verifyInline1(T const* results, intptr_t elt_n, int red_op, int rank_n, uint64_t seed,
              intptr_t elt_ix0, unsigned tolerance, int64_t* bad_elt_n,
              cudaStream_t stream, int block_n)
{
#    define CASE_OP(op)                                                                  \
        if(rank_n == 1)                                                                  \
            verifyInline2<T, Uint><<<block_n, 512, 0, stream>>>(                         \
                (T const*) results, elt_n, ReduceNil(), rank_n, seed, elt_ix0,           \
                tolerance, bad_elt_n);                                                   \
        else                                                                             \
            verifyInline2<T, Uint>                                                       \
                <<<block_n, 512, 0, stream>>>((T const*) results, elt_n, op, rank_n,     \
                                              seed, elt_ix0, tolerance, bad_elt_n);      \
        break;
    switch(red_op)
    {
        case ncclSum: CASE_OP(ReduceSum())
...
{
    bool floating =
        elt_ty == ncclFloat16 || elt_ty == ncclFloat32 || elt_ty == ncclFloat64;
#    if HAVE_ncclBfloat16
...
#    if HAVE_ncclAvg
    if(floating && red_op == ncclAvg) tolerance = calcSumFloatTolerance(rank_n, elt_ty);
#    endif
...
#    define CASE_TY(T, Uint)                                                             \
        {                                                                                \
            if(expected != nullptr)                                                      \
            {                                                                            \
                verifyPrepared<<<block_n, 512, 0, stream>>>(                             \
                    (Uint const*) results, (Uint const*) expected, elt_n, tolerance,     \
                    bad_elt_n);                                                          \
            }                                                                            \
            else                                                                         \
            {                                                                            \
                verifyInline1<T, Uint>((T const*) results, elt_n, red_op, rank_n, seed,  \
                                       elt_ix0, tolerance, bad_elt_n, stream, block_n);  \
            }                                                                            \
        }                                                                                \
        break;
    switch(elt_ty)
...
__device__ void
sweep2(int ty, char const* tyname, Op op, char const* opname, int rank_n)
{
    // if(!std::is_same<T,half>::value) return;
    // if(!std::is_same<Op,ReduceProd>::value) return;
    // if(rank_n!=3) return;

    unsigned tolerance = !IsIntegral<T>::value && std::is_same<Op, ReduceAvg>::value
                             ? calcSumFloatTolerance(rank_n, ty)
                             : 0;
...
template <typename T>
__device__ void
sweep1(int ty, char const* tyname)
{
    for(int i = 0; i < 10; i++)
    {
        int rank_n = (1 << i) + i;
